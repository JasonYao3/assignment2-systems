...
def benchmark_attention():
    ...
    print(
        f"{'d_model':<10} | {'seq_len':<10} | {'Fwd (ms)':<10} | {'Bwd (ms)':<10} | {'Mem (MB)':<10} | {'Status'}"
    )
    print("-" * 75)
    ...
    for d in d_models:
        for s in seq_lens:
            ...
            try:
                # TO DO: Warmup (5 times) and Time Forward Pass (100 times)
                for _ in range(5):
                ...
                # TO DO: Get peak memory
                mem_mb = get_memory_info(device)

                # TO DO: Time Backward Pass (100 times)
                ...
                print(
                    f"{d:<10} | {s:<10} | {fwd_time:<10.3f} | {bwd_time:<10.3f} | {mem_mb:<10.2f} | {'Ok'}"
                )

            except RuntimeError as e:
                # If memory explodes, PyTorch throws a RuntimeError.
                print(
                    f"{d:<10} | {s:<10} | {'OOM':<10} | {'OOM':<10} | {'OOM':<10} | {'Failed'}"
                )
